...
const { get_open_drivers, get_open_riders, get_driver, get_rider, insert_driver, insert_rider, update_rider_with_driver, update_rider_remove_driver, update_driver_seats } = require('./database');
...
app.get('/test', function(request, response) {
	let output = "";
	get_open_riders('Slackathon', 9, true, (err, res) => {
		if (err) throw err;
		for (let row of res.rows) {
			console.log(JSON.stringify(row));
			output += JSON.stringify(row) + "\n"
